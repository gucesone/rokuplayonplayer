...
'******************************************************
'Get total seconds from time string (HH:MM:SS)
'******************************************************
Function secondsFromString(time As String) As Integer
	seconds = 0
	...
	while t <> invalid
		if (state = 0)
			seconds = seconds + (60*60*(t.toint()))
		else if (state = 1)
			seconds = seconds + (60 * (t.toint()))
		else if (state = 2)
			seconds = seconds + (t.toint())
		endif 
...
'******************************************************
'Get first XML subelement
'
'return invalid if not found, else the element
'******************************************************
Function GetFirstXMLElement(xml As Object) As dynamic
...
Function ExtractStrFromMatch(str As String, match As String, matchIndex As Integer) As String
	r = CreateObject("roRegex", match, "")
	result = r.Match(str)
	if result.Count() > matchIndex then
		print "ExtractStrFromMatch: matched str = " result[matchIndex]
		return result[matchIndex]
	endif
	
	print "ExtractStrFromMatch: no match" 
	return ""
End Function
...
Function goodFirmwareVersion(minMajor=0, minMinor=0, minBuild=0) As boolean
    ...
    Dbg("goodFirmwareVersion: firmware version found = " + version)

    major = Val(Mid(version, 3, 1))
    minor = Val(Mid(version, 5, 2))
    build = Val(Mid(version, 8, 5))

    Dbg( "goodFirmwareVersion: major version = ",major)
    Dbg( "goodFirmwareVersion: minor version = ",minor)
    Dbg( "goodFirmwareVersion: build number  = ",build)
